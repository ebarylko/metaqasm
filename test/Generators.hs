...
-- Takes a formatter for the number of elements in a register collection and
-- returns a formatter that generates a type annotation stating that
-- a gate parameter is a quantum register collection
qubitRegCollAnnotation :: RegAccessFormatter -> RegAccessFormatter
...
conditionalGateExecution = formatToString potentialGateExec <$> conditionalGateInfo
  where
    ...
    potentialGateExec = execGateIf expectedBitVal gateApp

    gateApp  = viewed gateData hadamardApp'
...
-- Generates a program that executes a gate if a bit b
-- has the same value as a sum of indices I + I'
programThatExecsGateIfBitEqualsSum :: Gen MetaQasmProgram
programThatExecsGateIfBitEqualsSum = formatToString conditionalGate <$> conditionalGateInfo
  where
...
-- less than n arguments in the body
circuitFamilyThatAppliesNAryGateToLessThanNArgs :: Gen MetaQasmProgram
circuitFamilyThatAppliesNAryGateToLessThanNArgs
